...
// his code listens for changes on each checkbox and ensures that when one is checked, all others are unchecked, which means only one list will be open at a time.

document.querySelectorAll('.list-container input[type="checkbox"]').forEach(checkbox => {
    checkbox.addEventListener('change', function() {
        // Close all other lists
        document.querySelectorAll('.list-container input[type="checkbox"]').forEach(cb => {
            if (cb !== this) {
                cb.checked = false;
            }
        });
